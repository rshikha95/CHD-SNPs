...
if(length(install_missing_packages)) install.packages(install_missing_packages)
...
# Step 1: Download the SNPs data from GWAS-catalog and Clinvar database using 
# following links: 

# (a) GWAS-Catalog data link: 
# https://www.ebi.ac.uk/gwas/docs/file-downloads
# (b) ClinVar Database link: https://ftp.ncbi.nlm.nih.gov/pub/clinvar/tab_delimited/variant_summary.txt.gz

# Step 2: Load the GWAS-catalog and ClinVar genetic variation files
...
chd_traits<- read.table("CHD_traits.txt") # Download CHD-traits list from 
...
```{shell Integration of Heart organogenesis epigenetic data from Cotney's Lab}
...
# Step 2: Convert the file format of downloaded data from .bigWig to .wig and
# then convert .wig files to .bed
...
# Step 3: Perform intersections of noncoding CHD-SNPs containing 150bp elements 
# with  Coteny's data in Step 2
...
```{r Visualization of correlated replicates from cotney's data}
...
# Group unique enhancers based on stage time points such as early, intermediate,
# late and their combinations
...
library(RMariaDB)
library(Biostrings)  # Required for the DNAStringSet and writeXStringSet functions
library(BSgenome.Hsapiens.UCSC.hg38)
library(rtracklayer)
